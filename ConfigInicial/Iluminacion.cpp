...
*
*		      	Práctica 7: Iluminación       Fecha de entrega: 4 de octubre de 2024*/
// Std. Includes
#include <string>
...
glm::vec3 puntoAlto(0.0f,0.0f,0.0f);
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Materiales e Iluminacion: Diana Jimenez", nullptr, nullptr);
    ...
    Model estambre((char*)"Models/fabric_balls.obj");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        else
        {
            ...
            glUniform3f(glGetUniformLocation(lightingShader.Program, "light.ambient"), 0.7f, 0.7f, 0.3f);
            glUniform3f(glGetUniformLocation(lightingShader.Program, "light.diffuse"), 0.4f, 0.7f, 0.3f);
            glUniform3f(glGetUniformLocation(lightingShader.Program, "light.specular"), 0.3f, 0.3f, 0.4f);
        ...
        glUniform3f(glGetUniformLocation(lightingShader.Program, "material.diffuse"), 0.7f, 0.2f, 0.8f);
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
    ...
    if (keys[GLFW_KEY_O])
    {
        if (lightPos.x+puntoAlto.x<=1.5f and lightPos.y+puntoAlto.y<=3.0f ) {
            //movelightPos += 0.1f;
            puntoAlto.x += 0.1f;
            puntoAlto.y += 0.1f;
            puntoAlto.z += 0.1f;
           ...
           if (lightPos.x+puntoAlto.x<=3.0f and lightPos.y+puntoAlto.y<=-2.0f ) {
                puntoAlto.x += 0.1f;
                puntoAlto.y -= 0.1f;
                puntoAlto.z += 0.1f;
            }
